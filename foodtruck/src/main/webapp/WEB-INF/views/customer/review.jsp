						...
						<tbody>
							<c:forEach var="i" items="${rlist }">
								<tr>
									...
									<td>${i.brandname}</td>
									<td><c:if test="${i.review_score==1 }">
											<a id="on" class="on">★</a>
										</c:if> <c:if test="${i.review_score==2 }">
											<a id="on" class="on">★★</a>
										</c:if> <c:if test="${i.review_score==3 }">
											<a id="on" class="on">★★</a>
										</c:if> <c:if test="${i.review_score==4 }">
											<a id="on" class="on">★★★★</a>
										</c:if> <c:if test="${i.review_score==5 }">
											<a id="on" class="on">★★★★★</a>
										</c:if></td>
									...
									<td><a
										href="/truck/send/?truck_code=${i.truck_code}&send=1&review_code=${i.review_code}"><input
											type="button" class="modify" value="수정/삭제"></a> <input
										type="hidden" value="${i.truck_code }"></td>
								</tr>
							</c:forEach>

...
<script>
	$(document).ready(function() {
				...
				for (var i = 0; i < a.length; i++) {
					...
					$("<td></td>").text(a[i].request_truck_count + "명").appendTo(tr);
					if(a[i].request_status ==0){
						//status = "모집중";
						var c = $("<td></td>").text("모집중").appendTo(tr);
						$("<button></button>").text("신청목록").addClass("request_code").appendTo(c);
						$("<input />").attr({value :  a[i].request_code ,type : 'hidden'}).appendTo(c);
					}else{

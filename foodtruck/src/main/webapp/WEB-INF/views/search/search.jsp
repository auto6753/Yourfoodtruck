...
<c:forEach var="list" items="trucklist">
...
/* 	 	for(var i=0;i<data.positions.length;i++){
	...
	var list = [];
	for(var i=0;i<xarry.length;i++){
		var a = getDistanceFromLatLonInKm(lat1,lag1,xarry[i],yarry[i]); 
		a = a.toPrecision(3);
		a = a*1000;
		if(a<=1000){
			alert("추가");
			list.push("d");//${"list[i].name"}
		}else{
			alert("안추가");
		}
	}
	...
	var a = getDistanceFromLatLonInKm(lat1,lag1,lat2,lng2);
	a = a.toPrecision(3);
	a = a*1000;
	alert(a);
